...
static bool validate_kheap_table(void* kheap_start, void* kheap_end, struct KHEAP_ENTRY_TABLE* entry_table) {
...
static int kheap_create(struct KHEAP* kheap,
                 ...
                 struct KHEAP_ENTRY_TABLE* entry_table) 
{
    ...
    if (!validate_kheap_table(kheap_physical_start_addr, kheap_physical_end_addr, entry_table)) {
        return -STATUS_INVALID_ARG;
    }

    size_t kheap_table_size = sizeof(KHEAP_BLOCK_TABLE_ENTRY) * entry_table->num_entries;

    memset(entry_table->entries, KHEAP_BLOCK_TABLE_ENTRY_FREE, kheap_table_size);
...
int kheap_init()
{
    ...
    int res = kheap_create(&kheap, (void*)KHEAP_ADDR, kheap_end, &kheap_entry_table);
...
static void* kheap_block_to_addr(int block_index) {
    return (void*) ((size_t)kheap.kheap_physical_start_addr + (block_index * KHEAP_BLOCK_SIZE));
}

static size_t kheap_addr_to_block_index(void* addr) {
    return ( (size_t) addr - (size_t) kheap.kheap_physical_start_addr ) /  KHEAP_BLOCK_SIZE;
}
...
void* kmalloc(size_t size) {
    ...
    KHEAP_BLOCK_TABLE_ENTRY entry = KHEAP_BLOCK_TABLE_ENTRY_TAKEN | KHEAP_BLOCK_IS_FIRST;
    ...
    // search_start = first_alloc_block + num_blocks; // next time we start searching from here
